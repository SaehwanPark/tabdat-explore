...
entry uses only `read`, `write`, `control`, `plot`, or `unknown`; emitted category tuples are ordered
exactly `read`, `write`, `control`, `plot`, `unknown`. Categories describe possible top-level effects,
including delegated `run`/`by` behavior and output/artifact writes; they do not inspect data or plan
...
| `isid` | Assert that key variables uniquely identify rows | `isid patient_id visit` |
...
| `gsort` | Stable-sort active rows by explicit directions | `gsort -date +patient_id` |
...
| `label` | Set, list, and persist variable/value labels | `label variable age "Age in years"` |
...
| `doctor` | Inspect environment, core engines, and capability health | `doctor` |
