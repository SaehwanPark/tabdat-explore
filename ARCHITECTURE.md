...
- Phase 3 commands are executable: `codebook`, `missing`, `duplicates`, `isid`, `datasignature`, `assert`, `count`, `head`, `tail`, `keep`,
  `drop`, `select`, `sort`, `gsort`, `rename`, `generate`, `replace`, `tabulate`, `collapse`, and supported
  `by:` forms.
