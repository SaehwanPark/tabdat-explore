...
def test_help_topics_document_expression_domains() -> None:
  ...
  assert "exact `DECIMAL(38,0)` storage" in load_help_topic("generate")
  assert "overflow rows: N" in load_help_topic("generate")
  assert "numeric/string conversion is not" in load_help_topic("replace")
  assert "exact `DECIMAL(38,0)` storage" in load_help_topic("replace")
  assert "overflow rows: N" in load_help_topic("replace")
  assert "overflow rows: N" in load_help_topic("keep")
  assert "overflow rows: N" in load_help_topic("drop")
